...
class UsuarioController extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($buscar) {
            ...
            // Si no encontró resultados, carga todos los usuarios
            if (empty($usuarios)) {
                $usuarios = $usuarioModel->resetQuery()->findAll();
            }
        } else {
            $usuarios = $usuarioModel->findAll();
        }
    ...
    public function actualizar($id)
    {
        ...
        if (empty($data['password'])) {
            unset($data['password']); // No actualizar password si no se envía
        }

        $this->usuarioModel->update($id, $data);

        return redirect()->to('/usuarios')->with('msg', 'Usuario actualizado correctamente');
    }
